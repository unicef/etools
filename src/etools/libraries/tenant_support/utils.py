...
class every_country:
    ...
    def __enter__(self):
        ...
        for c in get_tenant_model().objects.exclude(schema_name='public').all():
...
def set_country(user, request):
    ...
    if country:
        try:
            country = get_tenant_model().objects.get(
                Q(name=country) |
