...
from etools.applications.audit.models import Auditor, UNICEFUser
...
from etools.applications.travel.models import Activity, ActivityActionPoint


class Command(BaseCommand):
    action_points_block = [
        'travel.activity.action_points',
        'travel.activityactionpoint.*',
    ]

    follow_up_editable_page = ['travel.activity']
    ...
    all_unicef_users = [unicef_user]  # [focal_point, unicef_user]
    ...
    def _update_permissions(self, role, perm, targets, perm_type, condition=None):
        ...
        condition = (condition or []) + [TravelModuleCondition()] + self.user_roles[role]
    ...
    def new_action_point(self):
        model = get_model_target(ActivityActionPoint)
    ...
    def handle(self, *args, **options):
        ...
        if self.verbosity >= 1:
            self.stdout.write(
                'Travel permissions updated ({}) -> ({}).'.format(
    ...
    def assign_permissions(self):
        ...
        # final_activity_condition = self.activity_status(
        #     Activity.STATUS_FINAL,
        # )
        # self.add_permissions(
        #     self.everybody,
        #     'view',
        #     self.report_block,
        #     condition=final_activity_condition
        # )
        self.add_permissions(
        ...
        opened_action_point_condition = self.action_point_status(
            ActivityActionPoint.STATUSES.open,
        )
        ...
        self.add_permissions(
            [self.action_point_assignee],
            'action',
            'travel.activityactionpoint.complete',
            condition=opened_action_point_condition
        )
