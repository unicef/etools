...
class TestLocationSiteAdmin(BaseTenantTestCase):
    ...
    @staticmethod
    def create_test_xlsx_with_errors():
        ...
        ws.append(['', '34.298401', '35.850361'])  # invalid row, no name, will raise error
        ws.append(['LOC: Meftah es Sellom_LBN34165', '34.99e', '33.517090'])  # invalid Coordinates, will raise error
    ...
    def test_import_file_invalid(self):
        self.sites_file = SimpleUploadedFile(
            'LocationSites.xlsx',
            self.create_test_xlsx_with_errors().read(),
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(LocationSite.objects.count(), 0, 'Error was raised')

    def test_import_file_valid(self):
        self.sites_file = SimpleUploadedFile(
            'LocationSites.xlsx',
            self.create_test_xlsx_valid().read(),
            content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(LocationSite.objects.count(), 4)
