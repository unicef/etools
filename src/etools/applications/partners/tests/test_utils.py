...
def setup_filtered_intervention_test_data(test_case, include_results_and_indicators=False):
    today = datetime.date.today()
    test_case.unicef_staff_2 = UserFactory(is_staff=True)
    test_case.partnership_manager_user_2 = UserFactory(
        is_staff=True, realms__data=[UNICEF_USER, PARTNERSHIP_MANAGER_GROUP]
    )
    test_case.partner_2 = PartnerFactory(organization=OrganizationFactory(name='Partner 3', vendor_number="VP2"))
    test_case.partner1_2 = PartnerFactory(organization=OrganizationFactory(name='Partner 4'))
    test_case.agreement_2 = AgreementFactory(partner=test_case.partner_2, signed_by_unicef_date=datetime.date.today())

    test_case.active_agreement_2 = AgreementFactory(
        partner=test_case.partner1_2,
        status='active',
        signed_by_unicef_date=datetime.date.today(),
        signed_by_partner_date=datetime.date.today()
    )
    ...
    test_case.intervention_2 = InterventionFactory(
        agreement=test_case.agreement_2,
        title='Intervention 1',
        status=Intervention.DRAFT,
    )
    test_case.intervention_2.unicef_focal_points.add(test_case.partnership_manager_user_2)
    test_case.intervention_2_2 = InterventionFactory(
        agreement=test_case.agreement_2,
        title='Intervention 2',
        document_type=Intervention.PD,
        status=Intervention.DRAFT,
    )
    test_case.intervention_2_2.unicef_focal_points.add(test_case.partnership_manager_user_2)
    test_case.active_intervention_2 = InterventionFactory(
        agreement=test_case.active_agreement_2,
        title='Active Intervention',
        document_type=Intervention.PD,
        ...
        unicef_signatory=test_case.unicef_staff_2,
        partner_authorized_officer_signatory=test_case.partner1_2.active_staff_members.all().first()
    )
    Intervention.objects.filter(id=test_case.active_intervention_2.id).update(modified=today - datetime.timedelta(days=730))
    test_case.active_intervention_2.unicef_focal_points.add(test_case.partnership_manager_user_2)
    test_case.reporting_requirement_2 = ReportingRequirementFactory(intervention=test_case.active_intervention_2)
    test_case.result_type_2 = ResultType.objects.get_or_create(name=ResultType.OUTPUT)[0]
    test_case.result_2 = ResultFactory(result_type=test_case.result_type_2)

    test_case.management_budget_2 = test_case.intervention_2.management_budgets
    test_case.partnership_budget_2 = test_case.intervention_2.planned_budget

    # set up two frs not connected to any interventions
    test_case.fr_1_2 = FundsReservationHeaderFactory(intervention=None, currency='USD')
    test_case.fr_2_2 = FundsReservationHeaderFactory(intervention=None, currency='USD')

    if include_results_and_indicators:
        # setup additional inicator/results
        test_case.result_2 = ResultFactory(name='A Result')
        ...
        test_case.applied_indicator_2 = AppliedIndicator.objects.create(
            indicator=test_case.indicator_blueprint_2,
            lower_result=test_case.lower_result_2,
        )
        test_case.applied_indicator_2.locations.add(LocationFactory(
            name='A Location',
            p_code='a-p-code')
        )
        test_case.disaggregation_2 = test_case.applied_indicator_2.disaggregation.create(name='A Disaggregation 2')

    test_case.file_type_attachment_2 = AttachmentFileTypeFactory(
        code="partners_intervention_attachment"
    )
    test_case.file_type_prc_2 = AttachmentFileTypeFactory(
        code="partners_intervention_prc_review"
    )
    test_case.file_type_pd_2 = AttachmentFileTypeFactory(
        code="partners_intervention_signed_pd"
    )
