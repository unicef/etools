    ...
    def programmatic_visits(cls, partner, event_date=None, update_one=False):
        ...
        if update_one and event_date:
            ...
            pv_year = TravelActivity.objects.filter(
                ...
                travels__end_datetime__year=timezone.now().year,
                partner=partner,
            )

            pv = pv_year.count()
            pvq1 = pv_year.filter(travels__end_datetime__month__in=[1, 2, 3]).count()
            pvq2 = pv_year.filter(travels__end_datetime__month__in=[4, 5, 6]).count()
            pvq3 = pv_year.filter(travels__end_datetime__month__in=[7, 8, 9]).count()
            pvq4 = pv_year.filter(travels__end_datetime__month__in=[10, 11, 12]).count()
