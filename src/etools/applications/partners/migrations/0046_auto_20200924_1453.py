...
def migrate_staff_members_to_fk(apps, schema_editor):
    ...
    for staff_member in PartnerStaffMember.objects.all():
        if staff_member.email.lower() in used_emails:
            # more than one staff member will be assigned to the user, add random prefix to email
            new_email = get_random_sequence() + '.' + staff_member.email
        ...
        staff_member.save()
        used_emails.append(staff_member.email.lower())
