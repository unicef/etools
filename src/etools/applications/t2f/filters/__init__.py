...
from etools.applications.t2f.serializers.filters import SearchFilterSerializer
...
class TravelActivityPartnerFilter(BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        return queryset.filter(partner__pk=view.kwargs['partner_organization_pk'])\
...
class TravelActivityInterventionFilter(BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        return queryset.filter(partnership__pk=view.kwargs['partnership_pk'])\
            .prefetch_related('travels')\
            .filter(travels__traveler=F("primary_traveler"))
