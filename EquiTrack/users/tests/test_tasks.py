...
from EquiTrack.tests.mixins import SCHEMA_NAME, TenantTestCase
...
class TestUserMapper(TenantTestCase):
    def setUp(self):
...
class TestSyncUsers(TenantTestCase):
    def setUp(self):
...
@skip("Issues with using public schema")
class TestMapUsers(TenantTestCase):
    def setUp(self):
...
class TestUserSynchronizer(TenantTestCase):
    def setUp(self):
