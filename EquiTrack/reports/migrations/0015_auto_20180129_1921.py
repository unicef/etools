...
def change_unit(apps, schema_editor):
    ...
    yesno_indicators = AppliedIndicator.objects.filter(indicator__unit='yesno').all()

    myprint('number of yesno indicators updated: ', yesno_indicators.count())

    for aind in yesno_indicators:
        aind.indicator.unit = IndicatorBlueprint.NUMBER
        aind.indicator.save()
        aind.target = 1
        aind.baseline = 0
        aind.save()
...
def finalize_migrations(apps, schema_editor):
    ...
    for aind in ainds:
        ...
        if intervention_sections > 1:
            myprint('many sections for this pd ', intervention.id, ' ', intervention.number, ' ', intervention.status)
            # TODO: Raise these
            #raise Exception("this intervention has more than 1 section")
        if intervention_sections < 1:
            myprint('no sections for this pd ', intervention.id, ' ', intervention.number, ' ', intervention.status)
    ...
    operations = [
        ...
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('active', models.BooleanField(default=False)),
                ('disaggregation', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='disaggregation_values', to='reports.Disaggregation')),
            ],
            options={
                'abstract': False,
            },
