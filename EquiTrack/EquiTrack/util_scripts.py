...
def result_chain_associate(country_name=None):
    ...
    for rc in ResultChain.objects.all():
        ...
        # create lower_indicator
        if rc.indicator:
            ind = Indicator.objects.get(id=rc.indicator.id)
            lind, created = LowerIndicator.objects.get_or_create(
                sector=rc.indicator.sector,
                result_structure=rc.indicator.result_structure,
                result=lrs,
                name=rc.indicator.name,
                code=rc.indicator.code,
                unit=rc.indicator.unit,
                total=rc.indicator.total,
                sector_total=rc.indicator.sector_total,
                current=rc.indicator.current,
                sector_current=rc.indicator.sector_current,
                assumptions=rc.indicator.assumptions,
                target=rc.indicator.target,
                baseline=rc.indicator.baseline
            )
            rc.lower_indicator = lind
        rc.save()

    # set parent of lower_result
    for rc in ResultChain.objects.all():
        if rc.result.parent:
            parent_result = rc.result.parent
            result_chain_parent = ResultChain.objects.filter(result=parent_result)[0]
            parent_lrs = result_chain_parent.lower_result
            rc.lower_result.parent = parent_lrs
