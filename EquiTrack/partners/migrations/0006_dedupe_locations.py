...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        deleted_locations = 0
        for pca in orm.PCA.objects.filter(amendment=True, original__isnull=False):
            original_locations = pca.original.locations.select_related('location').all()
            original_location_ids = [loc.location.id for loc in original_locations]
            for location in pca.locations.all():
                if location.location.id in original_location_ids:
                    location.delete()
                    deleted_locations += 1

        print "Deleted {} locations".format(deleted_locations)
    ...
    models = {
        ...
        u'partners.pca': {
            ...
            'unicef_managers': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'to': u"orm['auth.User']", 'null': 'True', 'blank': 'True'}),
