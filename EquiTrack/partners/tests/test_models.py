...
class TestRefNumberGeneration(TenantTestCase):
    ...
    def test_pca_ref_generation(self):
        ...
        # agreement numbering remains the same even if previous agreement is
        # deleted
    ...
    def test_other_agreement_types(self):
        ...
        for doc_type in [Agreement.MOU, Agreement.IC, Agreement.AWP, Agreement.SSFA]:
            ...
            self.assertEqual(agreement.reference_number,
                             self.text.format(doc_type))
    ...
    def test_pd_numbering(self):
        ...
        self.assertEqual(intervention1.reference_number,
                         pd_ref.format('PD', '01'))
        ...
        self.assertEqual(intervention2.reference_number,
                         pd_ref.format('PD', '02'))
        ...
        self.assertEqual(intervention2.reference_number,
                         pd_ref.format('PD', '02-01'))

        intervention3 = PartnershipFactory(
            partner=intervention1.partner,
            agreement=intervention1.agreement,
        )
        self.assertEqual(intervention3.reference_number,
                         pd_ref.format('PD', '03'))
...
class TestHACTCalculations(TenantTestCase):
    ...
    def test_planned_cash_transfers(self):
        ...
        self.assertEqual(self.intervention.partner.hact_values[
                         'planned_cash_transfer'], 60000)
...
class TestPartnerOrganizationModel(TenantTestCase):
    ...
    def test_micro_assessment_needed_high_risk(self):
        ...
        self.assertEqual(self.partner_organization.hact_values[
    ...
    def test_micro_assessment_needed_pct_over_100k(self):
        ...
        self.partner_organization.hact_values[
            "planned_cash_transfer"] = 100001.00
        self.partner_organization.save()
        assessment = Assessment.objects.create(
            partner=self.partner_organization,
            type="Micro Assessment",
    ...
    def test_audit_needed_extra_assessment_after_last(self):
        ...
        self.partner_organization.total_ct_cp = 500001.00
        self.partner_organization.save()
        PartnerOrganization.audit_needed(
            self.partner_organization, assessment2)
        self.assertEqual(self.partner_organization.hact_values['audits_mr'], 0)
...
class TestAgreementModel(TenantTestCase):
    ...
    def setUp(self):
        ...
        action.send(self.partner_organization,
                    verb="created", target=self.agreement)

    @skip("Fix this")
    def test_reference_number(self):
        year = datetime.datetime.today().year
        self.assertEqual(self.agreement.reference_number,
                         "/PCA{}01".format(year))
...
class TestInterventionModel(TenantTestCase):
    ...
    def test_days_from_submission_signed(self):
        self.intervention.submission_date = datetime.date(
            datetime.date.today().year - 1, 1, 1)
        self.intervention.signed_by_partner_date = datetime.date(
            datetime.date.today().year - 1, 5, 1)
        days = (self.intervention.signed_by_partner_date -
                self.intervention.submission_date).days
    ...
    def test_days_from_review_to_signed(self):
        self.intervention.submission_date = datetime.date(
            datetime.date.today().year - 1, 1, 1)
        self.intervention.review_date = datetime.date(
            datetime.date.today().year - 1, 2, 1)
        self.intervention.signed_by_partner_date = datetime.date(
            datetime.date.today().year - 1, 5, 1)
        days = (self.intervention.signed_by_partner_date -
                self.intervention.review_date).days
        # self.assertEqual(self.intervention.days_from_review_to_signed, days)

    # TODO relativedelta() returns 0, may be a bug in the model code
    def test_duration(self):
        self.intervention.start_date = datetime.date(
            datetime.date.today().year - 1, 1, 1)
        self.intervention.end_date = datetime.date(
            datetime.date.today().year + 1, 1, 1)
